    ...
    class RepairSystemPatch
    {
            ...
            [HarmonyArgument(2)] byte amount)
        {
            ...
            if (!Options.MadmateCanFixLightsOut.GetBool() && //Madmateが停電を直せる設定がオフ
               ...
               (player.Is(CustomRoles.Madmate) || player.Is(CustomRoles.MadMedic) || player.Is(CustomRoles.MadGuardian) || player.Is(CustomRoles.MadSnitch) || player.Is(CustomRoles.MadMayor) || player.Is(CustomRoles.SKMadmate))) //実行者がMadmateかMadGuardianかMadSnitchかSKMadmate)
                return false;
            if (!Options.MadmateCanFixComms.GetBool() && //Madmateがコミュサボを直せる設定がオフ
                systemType == SystemTypes.Comms && //システムタイプが通信室
                (player.Is(CustomRoles.Madmate) || player.Is(CustomRoles.MadMedic) || player.Is(CustomRoles.MadGuardian) || player.Is(CustomRoles.MadMayor))) //実行者がMadmateかMadGuardian)
                return false;
            ...
            if (player.Is(CustomRoles.Sheriff) || player.Is(CustomRoles.ImitatorSheriff) || player.Is(CustomRoles.Deputy) || player.Is(CustomRoles.Examiner) || player.Is(CustomRoles.NeutWitch) || player.Is(CustomRoles.Dracula) || player.Is(CustomRoles.Unseeable) || player.Is(CustomRoles.Investigator) || player.Is(CustomRoles.Wraith) || player.Is(CustomRoles.ImitatorEscort) || player.Is(CustomRoles.Escort) || player.Is(CustomRoles.Crusader) || player.Is(CustomRoles.Hustler) || player.Is(CustomRoles.TemplateRole) || player.Is(CustomRoles.Parasite) || player.Is(CustomRoles.Marksman) || player.Is(CustomRoles.BloodKnight) || player.Is(CustomRoles.Arsonist) || player.Is(CustomRoles.Werewolf) || player.Is(CustomRoles.AgiTater) || player.Is(CustomRoles.TheGlitch) || player.GetRoleType() == RoleType.Coven || player.Is(CustomRoles.PlagueBearer) || player.Is(CustomRoles.Pestilence) || player.Is(CustomRoles.Juggernaut) || ((player.Is(CustomRoles.Jackal) || player.Is(CustomRoles.Sidekick)) && !Options.JackalCanUseSabotage.GetBool()) || Main.Grenaiding)
            {
