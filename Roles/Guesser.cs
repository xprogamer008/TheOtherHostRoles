    ...
    public static class Guesser
    {
        ...
        private static void PirateEndGame(GameOverReason reason, bool showAd)
        {
            foreach (var pc in PlayerControl.AllPlayerControls)
            {
                ...
                if (pc.Is(CustomRoles.Sheriff) || pc.Is(CustomRoles.ImitatorSheriff) || pc.Is(CustomRoles.Deputy) || pc.Is(CustomRoles.Investigator) ||
                    (!(Main.currentWinner == CustomWinner.Arsonist) && pc.Is(CustomRoles.Arsonist)) || (Main.currentWinner != CustomWinner.Vulture && pc.Is(CustomRoles.Vulture)) || (Main.currentWinner != CustomWinner.Hustler && pc.Is(CustomRoles.Hustler)) || (Main.currentWinner != CustomWinner.TemplateRole && pc.Is(CustomRoles.TemplateRole)) || (Main.currentWinner != CustomWinner.Marksman && pc.Is(CustomRoles.Marksman)) || (Main.currentWinner != CustomWinner.Pirate && pc.Is(CustomRoles.Pirate)) ||
